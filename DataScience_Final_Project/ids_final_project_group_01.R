...
mydata <- mydata[mydata$helpful != 0, ] 
...
corpus_clean <- sapply(corpus_spell, clean_text)                     
changed_rows_clean <- which(corpus_spell != corpus_clean)            

affected_rows_clean <- data.frame(
  review_id = mydata$review_id[changed_rows_clean],                  
  original_review_text = corpus_spell[changed_rows_clean],          
  after_cleaning = corpus_clean[changed_rows_clean],                 
  stringsAsFactors = FALSE
)
